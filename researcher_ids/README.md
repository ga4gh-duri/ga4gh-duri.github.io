...
GA4GH Researcher Identity & Access Claims (aka RI Claims) specification aims to support data access policies within current and evolving data access governance systems. RI Claims defines a standard way of communicating 1) where users are from and assure they have been correctly identified, and 2) what data set access authorisations the user has based on either a user's (e.g. researcher) role, affiliation, or access status given by data access committees.

RI Claims from trusted services (NOTE: requires high security and availability, and a liable background organistation) can express data access rights given to specific users and specific datasets. Dataset access can either require a registration process (for the [Registered Access data access model](https://www.nature.com/articles/s41431-018-0219-y)) or a Controlled Access approval that many current data access governance processes use. Information is stored according to RI Claims, and consumed in the GA4GH ecosystem to provide data access in a technical environment to the identified individual. For example, to support Registered Access, RI Claims allows for the encoding and identification of users with Bona Fide Researcher Registered Access status. To simplify access to multiple data sets, different RI Claims that a user has acquired to authorise data set access may be bundled to form a RI Claims “Passport
